...
class GalleriesController < BoardController
  ...
  def index
      ...
      @galleries = Gallery.where(:gallery_category_id=>@gallery_category_id).order('id desc').page(params[:page]).per(10)
  ...
  def show
      ...
      @galleries = Gallery.where(:gallery_category_id=>@gallery_category_id).order('id').page(params[:page]).per(50)
